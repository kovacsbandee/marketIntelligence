...
class CompanyTableManager(PostgresManager):
    """
    A specialized handler for company data in a PostgreSQL database.
...
class CompanyDataManager(CompanyTableManager):
    """
    A specialized handler for company data in a PostgreSQL database.
    ...
    def insert_new_data(self, table: Type, rows: List[Dict]):
        ...
        with self.session_scope() as session:
            try:
                ...
                # Log each failing row for debugging
                for row in rows:
                    self.logger.error("Failing row for table %s: %s", table.__tablename__, row)
