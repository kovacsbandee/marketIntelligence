...
def update_company_base(tab, company_base_data):
    ...
    if not company_base_data:
        return dmc.Text("No company info loaded.", color="red"), False
...
def update_price_indicator(tab, start_date, end_date, price_data, dividends_data):
    if tab != "price-indicator":
        return dash.no_update, False
    if not price_data:
        return dmc.Text("No price data loaded.", color="red"), False
    ...
    try:
        ...
        return dmc.Text(f"Error generating plot: {e}", color="red"), False
